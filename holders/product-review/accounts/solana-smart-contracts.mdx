...
6. The formed batch is transferred to the Holders treasury wallet.
...
Treasury wallets are Holders wallets where funds ([USDC](https://www.circle.com/usdc) or [TON](https://ton.org/en)) are transferred after clearings are processed. These are also smart contracts, but with limited rights. Only Treasure authority can perform actions with treasury wallets; this smart contract has no Wallet authority.

### Available actions:

- Processing refunds and reversals from treasury wallets to client accounts
- Withdrawals from treasury wallets (only to wallet addresses specified in the whitelist)
...
- Updating whitelist for treasury withdrawals (adding or excluding addresses)
...
- `ctx_address_a`, `ctx_address_b`: owner and treasury addresses.
