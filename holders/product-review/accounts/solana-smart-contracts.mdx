...
USDC is a stablecoin operating on multiple blockchain networks, such as [Solana](https://solana.com/), [Ethereum](https://ethereum.org/), [Tron](https://tron.network/) and others. Each network uses a specific contract address to manage USDC transactions.\
USDC in Holders is available to all clients [with European residency](https://whales.enterprise.slack.com/docs/T045HLK3YLE/F09FNDP3PT7) who have completed onboarding on Walleexer.
...
1. The user selects a currency (TON or USDT) and initiates the creation of a new account, passing the corresponding command to the controller _\*_\* (Controller authority).
...
Treasury wallets are Holders wallets where funds (USDC or TON) are transferred after clearings are processed. These are also smart contracts, but with limited rights. Only Treasure authority can perform actions with treasury wallets; this smart contract has no Wallet authority.
