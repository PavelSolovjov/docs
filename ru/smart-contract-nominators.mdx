...
2. Saves updated participant data and base state
...
3. For each participant in the `members` dictionary:
   ...
   1. Saves participant data (`store_member()`)
1. Saves pool base state (`store_base_data()`)
...
3. Executes `member_stake_withdraw(stake)`:
   ...
   1. Places remainder in withdrawal queue (`ctx_member_pending_withdraw`)
   2. Returns immediate withdrawal amount and operation completion status
1. Determines recipient address:
...
2. Sends confirmation:
   ...
   1. in other cases: standard message with `op::stake_withdraw::response()` or `op::stake_withdraw::delayed()`
3. Saves updated member data and base state
...
3. For each participant in the `members` dictionary:
   ...
   2. Executes `member_accept_withdraw()`:
      ...
      2. resets `ctx_member_pending_withdraw` and `ctx_member_pending_withdraw_all` flag
   1. Saves participant data (`store_member()`)
1. Saves pool base state (`store_base_data()`)
