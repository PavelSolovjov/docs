...
- The message is signed locally with private key (via `WalletContract.createTransfer`). 3. Signing:
- Private key is extracted from secure storage after authentication (PIN/biometrics).
- Signing is performed locally on the device; private key never leaves the device. 4. Network transmission:
- Signed message is serialized to BOC (Bag of Cells).
- Sent to TON RPC node via `client.sendMessage(msg.toBoc())`.
- Application registers transaction as "pending" for status tracking. 5. Confirmation waiting:
...
- This requires additional SOL fee (~0.002–0.003 SOL). 
...
- Uses `getSignaturesForAddress` and `getTransaction`. 
  2. Token account monitoring:
