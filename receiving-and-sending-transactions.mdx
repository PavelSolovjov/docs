...
Sending token is sending an internal message to the sender's token-wallet address.

- Sender in Tonhub signs and sends an external TON transaction to their token-wallet with token transfer instruction.
...
- In the payload of message to token-wallet:
  - `op_code` = `0xf8a7ea5` (token Transfer)
  - token amount
  - recipient address
...
**Tokens** — minimum token transfer amount must exceed zero. Exact minimum value is determined by specific token characteristics (usually one smallest unit, e.g. 0.000001 for USDT). TON fee is paid by sender, except when using gasless transactions.
