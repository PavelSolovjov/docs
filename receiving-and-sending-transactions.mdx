...
- Blockhash is needed for transaction validity (usually valid ~60 seconds). 

2. Building transaction:

- `SystemProgram.transfer` instruction is created.

3. Adding comment (memo):
...
- Signing is performed on device.

5. Serialization and sending:
...
- Returns signature (base58), which is the transaction ID.
